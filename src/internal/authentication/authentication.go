...
func CreateDefaultUser(username, password string) (err error) {
	...
	if len(users) > 0 {
		err = createError(0o01)
		return
	}

	defaults := defaultsForNewUser(username, password)
...
func CreateNewUser(username, password string) (userID string, err error) {
	...
	checkIfTheUserAlreadyExists := func(username string, userData map[string]interface{}) (err error) {
		salt := userData["_salt"].(string)
		loginUsername := userData["_username"].(string)

		if SHA256(username, salt) == loginUsername {
			err = createError(0o20)
		}
...
func UserAuthentication(username, password string) (token string, err error) {
	...
	login := func(username, password string, loginData map[string]interface{}) (err error) {
		err = createError(0o10)

		salt := loginData["_salt"].(string)
		loginUsername := loginData["_username"].(string)
		loginPassword := loginData["_password"].(string)
...
func CheckTheValidityOfTheToken(token string) (newToken string, err error) {
	...
	err = createError(0o11)

	if v, ok := tokens[token]; ok {
		expires := v.(map[string]interface{})["expires"].(time.Time)
...
func SetDefaultUserData(defaults map[string]interface{}) (err error) {
	...
	for _, d := range allUserData {
		data := d.(map[string]interface{})["data"].(map[string]interface{})
		userID := d.(map[string]interface{})["_id"].(string)
...
func ChangeCredentials(userID, username, password string) (err error) {
	...
	if userData, ok := data["users"].(map[string]interface{})[userID]; ok {
		...
		salt := userData.(map[string]interface{})["_salt"].(string)
...
func saveDatabase(tmpMap interface{}) (err error) {
	...
	err = ioutil.WriteFile(database, []byte(jsonString), 0o600)
	if err != nil {
		return
	}
