...
func xTeVeAutoBackup() (err error) {
	archiv := "xteve_auto_backup_" + time.Now().Format("20060102_1504") + ".zip"
	var target string
	sourceFiles := make([]string, 0)
	oldBackupFiles := make([]string, 0)
	var debug string
...
func XteveBackup() (archiv string, err error) {
	...
	target := System.Folder.Temp + archiv
	sourceFiles := make([]string, 0)
...
func XteveRestoreFromWeb(input string) (newWebURL string, err error) {
	...
	archive := System.Folder.Temp + "restore.zip"
